...
import { Loader2, Eye, EyeOff, BrainCircuit, Sliders, Bookmark, Check, Volume2, Upload, FileText, Zap } from 'lucide-react';
...
export const VocabularyBuilder: React.FC<VocabularyBuilderProps> = ({ aiProvider }) => {
  ...
  // Load saved status on mount to check for duplicates
  useEffect(() => {
    ...
    try {
      if (mode === 'topic') {
        ...
        if (rawList.length > 20) {
           if (!confirm(`You are trying to generate cards for ${rawList.length} words. This might take a while. Continue?`)) {
             setLoading(false);
             return;
           }
        }

        const newWords = await generateVocabularyFromList(rawList, aiProvider);
        setWords(newWords);
      }
    } catch (error) {
      alert(`Failed to generate vocabulary using ${aiProvider}. Please check your connection or API key.`);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${
              mode === 'topic' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${
              mode === 'import' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            ...
            <div className="space-y-4">
              <div>
                ...
                <select 
                  ...
                  className="w-full p-2.5 rounded-lg border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-indigo-500 outline-none"
                >
                  {TOPICS.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Or Custom Topic</label>
                <input 
                  type="text" 
                  placeholder="e.g. Accounting, Gardening..."
                  value={customTopic}
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1 flex items-center gap-2">
                  <Sliders className="w-4 h-4" /> Difficulty Level
                </label>
          ...
          <div className="space-y-4 animate-in fade-in duration-300">
             <div>
                <label className="block text-sm font-medium text-slate-700 mb-2 flex items-center gap-2">
                  <FileText className="w-4 h-4" /> Paste List from Excel or Text
                </label>
                <textarea 
                  value={importText}
...
Ephemeral
Ubiquitous"
                  className="w-full p-4 rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-indigo-500 outline-none resize-none h-40 font-mono text-sm"
                />
                <p className="text-xs text-slate-500 mt-2">
                  Tip: You can copy a column directly from Excel and paste it here. Words can be separated by new lines or commas.
                </p>
             </div>
          </div>
        )}
        ...
        <button 
          ...
          disabled={loading || (mode === 'import' && !importText.trim())}
          className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-xl flex items-center justify-center gap-2 transition-colors disabled:opacity-50 shadow-sm hover:shadow-md"
        >
            ...
            <div key={index} className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden flex flex-col h-full transition-all hover:shadow-lg">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className={`p-1 rounded-full transition-colors ${
                      isSaved ? 'text-emerald-500 bg-emerald-50' : 'text-slate-400 hover:text-indigo-600 hover:bg-indigo-50'
                    }`}
              ...
              <div className="p-5 flex-1 space-y-4">
                 <div className="bg-amber-50 p-3 rounded-lg border border-amber-100">
                    <span className="text-xs font-bold text-amber-700 uppercase tracking-wider block mb-1">🧠 Memory Aid</span>
                ...
                {isRevealed ? (
                  <div className="space-y-4 animate-in fade-in duration-300">
                    <div>
                      <span className="text-xs font-semibold text-slate-400 uppercase">Meaning</span>
                      <p className="text-slate-800">{item.definition}</p>
                      <p className="text-slate-500 text-sm mt-1">{item.chineseTranslation}</p>
                    </div>
                    <div>
                      <span className="text-xs font-semibold text-slate-400 uppercase">Context ({item.context})</span>
                      <p className="text-indigo-900 bg-indigo-50 p-2 rounded text-sm border-l-4 border-indigo-400">
                  ...
                  <div className="flex-1 flex items-center justify-center py-8 opacity-40">
                    <p className="text-sm text-center">Tap eye icon to reveal definition</p>
                  </div>
                )}
              </div>
            </div>
